package com.unltm.distance.ui.components.wave;
...
public class AXWaveView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        boolean showWaves = currentState == null || currentState.supportWaves();
     ...
     */
    public @NonNull
    List<AXWaveDrawable> getAllWaveDrawables() {
    ...
    public void setState(AXWeavingState state) {
        ...
        if (state.width == 0 || state.height == 0) {
            if (state.width == 0)
                state.width = currentState != null ? currentState.width : findShaderSize();
            if (state.height == 0)
                state.height = currentState != null ? currentState.height : findShaderSize();
        }
